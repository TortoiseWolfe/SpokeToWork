...
The project will automatically use your repository name everywhere "SpokeToWork" appeared before.
...
4. Falls back to "SpokeToWork" if no git remote is found
...
2. **Run detection manually**:
   ...
   ```bash
   docker compose exec spoketowork node scripts/detect-project.js
   ```
...
3. **Check generated config**:
   ...
   ```bash
   docker compose exec spoketowork cat src/config/project-detected.json
   ```

4. **Clear and regenerate**:
   ```bash
   docker compose exec spoketowork rm -rf src/config/project-detected.*
...
3. **Default Values** - fallback to "SpokeToWork"
...
No more searching and replacing "SpokeToWork" across 100+ files! The auto-configuration system handles most settings based on your git repository. 🎉
