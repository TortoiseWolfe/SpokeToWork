...
docker compose exec spoketowork pnpm run type-check    # TypeScript
docker compose exec spoketowork pnpm run lint           # ESLint
docker compose exec spoketowork pnpm run format:check   # Prettier
docker compose exec spoketowork pnpm test              # Unit tests
docker compose exec spoketowork pnpm run test:coverage # Coverage
docker compose exec spoketowork pnpm run test:a11y     # Accessibility
```
...
```bash
docker compose exec spoketowork pnpm run e2e:accessibility
```
...
docker compose exec spoketowork node scripts/validate-structure.js
```
