...
docker compose exec spoketowork pnpm exec pa11y http://localhost:3000/messages --runner axe
docker compose exec spoketowork pnpm exec pa11y http://localhost:3000/messages/connections --runner axe
docker compose exec spoketowork pnpm exec pa11y http://localhost:3000/conversations --runner axe
```
...
docker compose exec spoketowork pnpm exec serve out -p 3000

# Terminal 3: Run Lighthouse
docker compose exec spoketowork pnpm exec lighthouse http://localhost:3000/messages \
...
# Analyze bundle
docker compose exec spoketowork pnpm exec next build --experimental-build-mode=compile
docker compose exec spoketowork ls -lh .next/static/chunks
```
...
2. Re-run build: `docker compose exec spoketowork pnpm run build`
