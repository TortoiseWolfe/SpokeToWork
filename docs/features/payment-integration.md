...
1. Click "New project"
2. Fill in details:
   - **Name**: `spoketowork-payments` (or your project name)
...
Edit `/home/turtle_wolfe/repos/SpokeToWork/.env` and add:
...
```bash
docker compose exec spoketowork supabase --version
...
docker compose exec spoketowork supabase link --project-ref $SUPABASE_PROJECT_REF --password $SUPABASE_DB_PASSWORD
```
...
```bash
# Push migrations
docker compose exec spoketowork supabase db push
...
```bash
# Generate types file
docker compose exec spoketowork sh -c 'supabase gen types typescript --linked > /app/src/lib/supabase/types.ts'

# Verify file created
docker compose exec spoketowork ls -lh /app/src/lib/supabase/types.ts
```
...
1. Click **Create App**
2. App Name: `SpokeToWork Payments`
