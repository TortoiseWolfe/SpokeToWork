...
**Status**: 📥 Inbox
**Created**: 2025-10-03
**Author**: SpokeToWork Development Team
...
### SpokeToWork Architecture Constraint
...
```bash
docker compose exec spoketowork pnpm add @supabase/supabase-js
docker compose exec spoketowork pnpm add -D supabase
```

**1.3 Initialize Supabase**

```bash
docker compose exec spoketowork npx supabase init
docker compose exec spoketowork npx supabase link --project-ref YOUR_PROJECT_REF
```
...
docker compose exec spoketowork npx supabase db push
```
...
docker compose exec spoketowork npx supabase functions deploy stripe-create-payment
docker compose exec spoketowork npx supabase functions deploy stripe-webhook
docker compose exec spoketowork npx supabase functions deploy paypal-create-order
docker compose exec spoketowork npx supabase functions deploy paypal-webhook
docker compose exec spoketowork npx supabase functions deploy payment-status
```
...
docker compose exec spoketowork pnpm run generate:component --name DonateButton --category atomic --hasProps true
docker compose exec spoketowork pnpm run generate:component --name CheckoutButton --category atomic --hasProps true
docker compose exec spoketowork pnpm run generate:component --name SubscribeButton --category atomic --hasProps true
docker compose exec spoketowork pnpm run generate:component --name PaymentConsentModal --category molecular --hasProps true
```
...
docker compose exec spoketowork npx supabase start

# Test Edge Functions locally
docker compose exec spoketowork npx supabase functions serve stripe-webhook

# Unit tests
docker compose exec spoketowork pnpm test src/utils/payment
docker compose exec spoketowork pnpm test src/components/payment
