...
This guide explains how to create new components in SpokeToWork following our enforced component structure standards.
...
docker compose exec spoketowork pnpm run audit:components

# Validate for CI/CD (exits with error if non-compliant)
docker compose exec spoketowork pnpm run validate:structure
```
...
docker compose exec spoketowork pnpm run migrate:components
```
...
```bash
docker compose exec spoketowork pnpm run audit:components -- --verbose
```
