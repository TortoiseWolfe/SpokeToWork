...
docker compose exec spoketowork pnpm dev
```
...
```bash
docker compose exec spoketowork pnpm test FontSwitcher
# Test fails ❌
```
...
# Run Storybook
docker compose exec spoketowork pnpm storybook
...
docker compose exec spoketowork pnpm format
```
