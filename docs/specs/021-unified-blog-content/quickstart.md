...
docker compose exec spoketowork pnpm add chokidar remark remark-html rehype-highlight crypto-js
docker compose exec spoketowork pnpm add -D @types/crypto-js

# Run migration
docker compose exec spoketowork pnpm run migrate:blog
```
...
docker compose exec spoketowork pnpm run test:migration
```
...
docker compose exec spoketowork pnpm run test:perf
...
```bash
# Test command
docker compose exec spoketowork pnpm test src/tests/integration/auto-generation.test.ts
```
...
```bash
# Test command
docker compose exec spoketowork pnpm test src/tests/integration/offline-sync.test.ts
```
...
```bash
# Test command
docker compose exec spoketowork pnpm test src/tests/integration/conflict-detection.test.ts
```
...
```bash
# Test command
docker compose exec spoketowork pnpm test src/tests/integration/migration.test.ts
```
...
```bash
# Test command
docker compose exec spoketowork pnpm test src/tests/integration/enhanced-processing.test.ts
```
...
docker compose exec spoketowork pnpm run dev:restart

# Check logs
docker compose logs spoketowork | grep chokidar
```
