...
docker compose exec spoketowork pnpm exec playwright test e2e/screenshot-current-state.spec.ts
```
...
docker compose exec spoketowork pnpm exec playwright test e2e/tests/blog-mobile-ux.spec.ts --project="iPhone 12"
...
docker compose logs spoketowork | grep -i error
```
...
docker compose exec spoketowork pnpm exec playwright test e2e/screenshot-after-quickfix.spec.ts
...
- [ ] Run `docker compose exec spoketowork pnpm run test:a11y:dev`
...
docker compose exec spoketowork pnpm list tailwindcss
...
docker compose exec spoketowork pnpm exec playwright install
...
docker compose exec spoketowork npx lighthouse http://localhost:3000 --preset=mobile --quiet
