...
- Command: `docker compose exec spoketowork pnpm run generate:component -- --name IdleTimeoutModal --category molecular --hasProps true`
...
- Run full test suite: `docker compose exec spoketowork pnpm test`
- Run TypeScript check: `docker compose exec spoketowork pnpm run type-check`
- Run ESLint: `docker compose exec spoketowork pnpm run lint`
...
docker compose exec spoketowork pnpm test src/lib/payments/__tests__/metadata-validator.test.ts
...
# Generate component
docker compose exec spoketowork pnpm run generate:component -- --name ComponentName --category molecular --hasProps true
```
