...
# Apply migration
docker compose exec spoketowork pnpm supabase:migrate
```
...
docker compose exec spoketowork pnpm test src/lib/payments/secure-payment-service.test.ts

# Test 2: OAuth CSRF protection
docker compose exec spoketowork pnpm test src/lib/auth/oauth-state.test.ts

# Test 3: Rate limiting
docker compose exec spoketowork pnpm test src/lib/auth/rate-limit-check.test.ts

# E2E Security Tests
docker compose exec spoketowork pnpm exec playwright test e2e/security/
```
