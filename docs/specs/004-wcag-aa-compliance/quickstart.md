...
docker compose exec spoketowork pnpm add -D @storybook/addon-a11y jest-axe @types/jest-axe
```
...
```bash
docker compose exec spoketowork pnpm run test:a11y:watch
```
...
docker compose exec spoketowork pnpm run test:a11y:components

# Generate HTML report
docker compose exec spoketowork pa11y-ci --reporter html

# Test specific URL
docker compose exec spoketowork pa11y http://localhost:3000/CRUDkit/themes --standard WCAG2AA
```
