...
**Constitution File**: `.specify/memory/constitution.md` (SpokeToWork Constitution v1.0.1 - 6 core principles)
...
# Verify
docker compose exec spoketowork npx supabase db execute \
...
```bash
docker compose exec spoketowork pnpm add react-easy-crop
```
...
```bash
# Unit tests
docker compose exec spoketowork pnpm test src/lib/avatar
docker compose exec spoketowork pnpm test src/components/atomic/AvatarUpload
