...
1. **Setup**:
   ...
   # Seed test data
   docker compose exec spoketowork pnpm run db:seed
   ```
...
2. **Create test payments**:
   ...
   docker compose exec spoketowork pnpm run db:seed:payments --count=10
   ```
...
docker compose exec spoketowork pnpm run test:load -- \
  --scenario=checkout \
  --users=500 \
  --duration=60s
...
docker compose exec spoketowork pnpm run test:stress -- \
...
```bash
# Reset test database
docker compose exec spoketowork pnpm run db:reset
