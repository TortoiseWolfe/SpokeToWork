...
# Initialize project
cd /home/turtle_wolfe/repos/SpokeToWork
supabase init
...
```bash
docker compose exec spoketowork pnpm add @supabase/supabase-js @stripe/stripe-js dexie resend

# Dev dependencies for testing
docker compose exec spoketowork pnpm add -D @supabase/supabase-js @types/stripe stripe
```
...
docker compose exec spoketowork npx supabase gen types typescript --local > src/lib/supabase/types.ts
```
...
```bash
docker compose exec spoketowork supabase db reset
docker compose exec spoketowork supabase db seed
```
...
docker compose exec spoketowork supabase db inspect
```
...
docker compose exec spoketowork supabase functions deploy stripe-webhook

# Deploy PayPal webhook
docker compose exec spoketowork supabase functions deploy paypal-webhook

# Deploy email sender
docker compose exec spoketowork supabase functions deploy send-payment-email

# List deployed functions
docker compose exec spoketowork supabase functions list
```
...
docker compose exec spoketowork supabase secrets set STRIPE_SECRET_KEY=sk_test_xxx
docker compose exec spoketowork supabase secrets set STRIPE_WEBHOOK_SECRET=whsec_xxx
docker compose exec spoketowork supabase secrets set PAYPAL_CLIENT_SECRET=xxx
docker compose exec spoketowork supabase secrets set RESEND_API_KEY=re_xxx

# List secrets (values hidden)
docker compose exec spoketowork supabase secrets list
```
...
```bash
docker compose exec spoketowork pnpm run generate:component PaymentButton -- \
  --category atomic \
  --hasProps true \
  --withHooks true
```
...
```bash
docker compose exec spoketowork pnpm run generate:component PaymentConsentModal -- \
  --category atomic \
  --hasProps true
```
...
```bash
docker compose exec spoketowork pnpm run generate:component PaymentDashboard -- \
  --category organisms \
  --hasProps false \
  --withHooks true
```
...
```bash
docker compose exec spoketowork pnpm run generate:component PaymentHistory -- \
  --category molecular \
  --hasProps true
```
...
```bash
docker compose exec spoketowork pnpm run generate:component SubscriptionCard -- \
  --category molecular \
  --hasProps true
```
...
```bash
docker compose exec spoketowork pnpm run test:a11y:dev
```
...
docker compose exec spoketowork pnpm run start

# Run Lighthouse
docker compose exec spoketowork npx lighthouse http://localhost:3000/dashboard/payments --output=html --output-path=./lighthouse-report.html
```
