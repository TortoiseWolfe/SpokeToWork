...
docker compose exec spoketowork node -e "console.log(process.env.NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY)"
```
...
```bash
# Run component tests
docker compose exec spoketowork pnpm test ContactForm

# Run all form-related tests
docker compose exec spoketowork pnpm test -- --testPathPattern=forms
