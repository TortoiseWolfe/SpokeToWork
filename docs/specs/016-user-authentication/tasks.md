...
docker compose exec spoketowork pnpm run generate:component -- --name ComponentName --category atomic --hasProps true

# Testing (TDD workflow)
docker compose exec spoketowork pnpm test                    # Run all tests
docker compose exec spoketowork pnpm test:coverage           # Coverage report
docker compose exec spoketowork pnpm exec playwright test    # E2E tests
docker compose exec spoketowork pnpm run test:a11y:dev       # Accessibility tests

# Code quality
docker compose exec spoketowork pnpm run type-check          # TypeScript validation
docker compose exec spoketowork pnpm run lint                # ESLint
docker compose exec spoketowork pnpm run test:suite          # Full test suite
