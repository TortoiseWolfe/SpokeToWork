...
const mockGetActiveRouteCompanyIds = vi
  .fn()
...
function Probe() {
  const { selectedCompanyId, selectCompany, clearSelection } =
    useCompanyWorkspace();
  return (
    <div>
...
function RouteIdsProbe() {
  const { activeRouteCompanyIds, refreshRouteCompanyIds } =
    useCompanyWorkspace();
  return (
    <div>
      <div data-testid="ids">
        {[...activeRouteCompanyIds].sort().join(',') || 'empty'}
      </div>
...
describe('CompanyWorkspaceContext — selection', () => {
  ...
  it('throws when used outside provider', () => {
    ...
    expect(() => render(<Probe />)).toThrow(
      'useCompanyWorkspace must be used within'
    );
    spy.mockRestore();
  });
});
