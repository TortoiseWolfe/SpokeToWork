...
export function useRouteAnalytics(): RouteAnalytics {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const metros = (metrosRes.data ?? []) as Array<{ id: string; name: string }>;
      ...
      const routesPerMetro: MetroRouteCount[] = Array.from(byMetro.entries())
        .map(([metroId, count]) => ({
          metroId,
          metroName: metroId == null ? 'Unassigned' : metroName.get(metroId) ?? metroId,
          count,
        }))
        .sort((a, b) => b.count - a.count || a.metroName.localeCompare(b.metroName));
