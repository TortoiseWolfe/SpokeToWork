...
// E2E test users (seeded in global-setup.ts)
const TEST_USER_PRIMARY = process.env.TEST_USER_PRIMARY_ID!;
const TEST_USER_SECONDARY = process.env.TEST_USER_SECONDARY_ID!;
...
describe('worker discoverability RLS', () => {
  ...
  beforeAll(async () => {
    ...
    admin = createAnon<any>(url, serviceKey, { auth: { persistSession: false } });
    ...
    const { data, error } = await admin
      .from('user_skills')
      .upsert({ user_id: TEST_USER_SECONDARY, skill_id: COURIER_UUID, is_primary: true },
