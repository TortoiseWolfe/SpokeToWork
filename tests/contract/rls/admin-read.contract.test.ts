...
const migration = readFileSync(
  resolve(process.cwd(), 'supabase/migrations/20251006_complete_monolithic_setup.sql'),
  'utf8',
);
...
describe('Admin cross-user read RLS policies', () => {
  ...
  it('private_companies has admin SELECT policy', () => {
    expect(migration).toContain('"Admin can view all private companies" ON private_companies');
  ...
  it('user_company_tracking has admin SELECT policy', () => {
    expect(migration).toContain('"Admin can view all tracking" ON user_company_tracking');
  });

  it('bicycle_routes has admin SELECT policy (DO-block form)', () => {
    expect(migration).toContain("policyname = 'bicycle_routes_admin_select'");
    expect(migration).toContain('bicycle_routes_admin_select ON bicycle_routes FOR SELECT');
  });
  ...
  it('route_companies has admin SELECT policy (DO-block form)', () => {
    ...
    expect(migration).toContain('route_companies_admin_select ON route_companies FOR SELECT');
  });
