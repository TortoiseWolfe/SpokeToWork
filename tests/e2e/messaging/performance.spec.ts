...
async function openConversation(page: Page) {
  ...
  try {
    ...
    // Thread container not visible — reload and retry (auth/encryption may have blocked)
    await page.reload();
    await dismissCookieBanner(page);
    await completeEncryptionSetup(page);
...
test.describe('Virtual Scrolling Performance', () => {
  ...
  test('T166: Performance with 1000 messages - scrolling FPS', async ({
    ...
    // Wait for scroll animation to actually reach the bottom.
    // WebKit smooth scroll takes 5-10s on CI — don't use a fixed wait,
    // poll until scrollTop is within 50px of the bottom.
    await page.waitForFunction(
      () => {
        const el = document.querySelector('[data-testid="message-thread"]');
        if (!el) return false;
