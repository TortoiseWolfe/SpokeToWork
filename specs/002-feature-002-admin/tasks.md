...
- [x] T001 [P] Add admin environment variables to `.env`
  ...
  ```bash
  TEST_USER_ADMIN_EMAIL=admin@spoketowork.com
...
- [x] T005 Add admin profile seed to `supabase/migrations/20251006_complete_monolithic_setup.sql`
  ...
  VALUES ('00000000-0000-0000-0000-000000000001', 'spoketowork', 'SpokeToWork', TRUE)
...
- [x] T007 Execute migration via Supabase Management API
  ...
  - Updated admin profile with username 'spoketowork'
...
**Goal**: Admin user "SpokeToWork" exists and can be authenticated
  ...
  describe('Admin User Setup', () => {
    it('should have admin profile in database', async () => {
      const { data } = await supabase
        ...
        .eq('id', ADMIN_USER_ID)
        .single();
      expect(data?.username).toBe('spoketowork');
    });
  });
  ```
...
- [x] T009 [US3] Verify admin profile exists via Supabase Management API
  ...
  - Confirmed: username='spoketowork', display_name='SpokeToWork', welcome_message_sent=true
...
**Goal**: New users receive encrypted welcome message from SpokeToWork on first login
