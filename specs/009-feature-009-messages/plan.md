...
docker compose exec spoketowork pnpm run type-check  # No any type errors
docker compose exec spoketowork pnpm run lint        # No ESLint warnings
docker compose exec spoketowork pnpm test            # All tests pass

# Final verification
docker compose exec spoketowork pnpm run build       # Production build clean
```
...
| SC-006: A11y passes            | `docker compose exec spoketowork pnpm run test:a11y`                        |
