...
```bash
docker compose exec spoketowork pnpm test src/lib/auth/oauth-utils.test.ts
```
...
```bash
# Unit tests
docker compose exec spoketowork pnpm test oauth-utils

# E2E test (if available)
docker compose exec spoketowork pnpm exec playwright test welcome-message
```
